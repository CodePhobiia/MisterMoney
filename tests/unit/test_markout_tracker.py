...
def test_record_and_update():
    ...
    fid = tracker.record_fill("tok-1", "cid-1", 0.55, "BUY", 0.56)
    tracker.update_markout(fid, current_mid=0.57, horizon="5s")

    pm = tracker._pending_by_id[fid]
...
def test_buy_adverse_selection():
    ...
    tracker = MarkoutTracker()
    fid = tracker.record_fill("tok-1", "cid-1", 0.55, "BUY", 0.56)
    tracker.update_markout(fid, current_mid=0.50, horizon="5s")

    pm = tracker._pending_by_id[fid]
    assert pm.markout_5s == pytest.approx(-0.05)


def test_sell_adverse_selection():
    """SELL fill where price rises should give negative markout."""
    tracker = MarkoutTracker()
    fid = tracker.record_fill("tok-1", "cid-1", 0.55, "SELL", 0.54)
    tracker.update_markout(fid, current_mid=0.60, horizon="5s")
...
def test_ewma_as_cost():
    """Multiple fills with 5m markouts should converge EWMA AS cost."""
    tracker = MarkoutTracker(ewma_halflife=5)
    for i in range(10):
        fid = tracker.record_fill("tok-1", "cid-1", 0.50, "BUY", 0.50)
        tracker.update_markout(fid, current_mid=0.48, horizon="5m")
...
def test_spread_capture():
    ...
    tracker = MarkoutTracker()
    fid_buy = tracker.record_fill("tok-1", "cid-1", 0.48, "BUY", 0.50)
    fid_sell = tracker.record_fill("tok-2", "cid-1", 0.52, "SELL", 0.50)

    assert tracker.get_spread_capture(fid_buy) == pytest.approx(0.02)
    assert tracker.get_spread_capture(fid_sell) == pytest.approx(0.02)


def test_spread_capture_out_of_bounds():
    """Spread capture for missing fill_id returns 0.0."""
    tracker = MarkoutTracker()
    assert tracker.get_spread_capture(-1) == 0.0
    assert tracker.get_spread_capture(999) == 0.0


def test_update_markout_missing_id():
    """Update markout for missing fill_id is a no-op."""
    tracker = MarkoutTracker()
    tracker.update_markout(-1, current_mid=0.50, horizon="5s")
    tracker.update_markout(999, current_mid=0.50, horizon="5s")
...
def test_pending_cap():
    """Pending dict should be capped at 5000 entries."""
    tracker = MarkoutTracker()
    for i in range(5010):
        tracker.record_fill("tok", "cid", 0.50, "BUY", 0.50)
    assert len(tracker._pending_by_id) == 5000
