...
from v3.calibration.decay import HALF_LIVES, decay_signal, is_signal_expired
from v3.calibration.route_models import CalibrationManager, RouteCalibrator
...
def test_conformal_intervals_cold_start():
    ...
    # Should be ~0.4 in cold start
    assert (p_high - p_low) >= 0.35, (
        f"Interval too narrow: {p_high - p_low}"
    )
...
def test_signal_decay():
    ...
    p_t_old = decay_signal(
        p_raw, market_mid,
        age_seconds=5 * half_life,
        source_staleness_seconds=0,
        route=route,
    )
    ...
    p_stale = decay_signal(
        p_raw, market_mid,
        age_seconds=half_life,
        source_staleness_seconds=half_life,
        route=route,
    )
    assert p_stale < p_t_half, f"Stale signal should decay more: {p_stale} vs {p_t_half}"

    print(
        f"✓ Signal decay: t=0: {p_t0:.4f},"
        f" t=half_life: {p_t_half:.4f},"
        f" t=5*half_life: {p_t_old:.4f}"
    )
...
def test_signal_expiration():
    ...
    for route, half_life in HALF_LIVES.items():
        ...
        assert not is_signal_expired(half_life, route), (
            f"{route}: Signal at half_life should not expire"
        )

        # Old signal should be expired
        # (lambda < 0.05 means age > -ln(0.05) * half_life)
        # 10% beyond boundary to avoid floating point issues
        expiry_age = -math.log(0.05) * half_life * 1.1
        assert is_signal_expired(expiry_age, route), (
            f"{route}: Signal should expire at {expiry_age}s"
        )

        print(
            f"✓ {route}: expires at"
            f" {expiry_age:.0f}s ({expiry_age / 60:.1f} min)"
        )
...
async def test_v3_consumer():
    ...
    expired_signal = FairValueSignal(
        ...
        generated_at=datetime.now(UTC) - timedelta(hours=5),  # Very old for numeric route
        p_calibrated=0.75,
        uncertainty=0.18,
        hurdle_met=True,
...
def test_calibration_manager():
    ...
    beta_rounded = [round(b, 4) for b in calibrator.beta]
    print(
        f"✓ Trained with {calibrator.resolved_market_count}"
        f" markets, beta={beta_rounded}"
    )
