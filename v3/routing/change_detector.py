...
class ChangeDetector:
    ...
    async def needs_refresh(self, 
                           condition_id: str, 
                           market: MarketMeta) -> Optional[ChangeEvent]:
        """
        ...
        if avg_volume > 0 and market.volume_24h > avg_volume * VOLUME_SPIKE_MULTIPLIER:
            ...
            return ChangeEvent(
                ...
                condition_id=condition_id,
                payload={
                    "reason": f"Volume spike detected ({market.volume_24h / avg_volume:.1f}x average)",
                    "volume_24h": market.volume_24h,
                    "avg_volume": avg_volume,
                },
            )
        ...
        if time_to_end.total_seconds() < APPROACHING_RESOLUTION_HOURS * 3600:
            ...
            if age_seconds > 600:  # 10 minutes
                ...
                return ChangeEvent(
                    ...
                    condition_id=condition_id,
                    payload={
                        "reason": f"Approaching resolution ({time_to_end.total_seconds() / 3600:.1f}h remaining)",
                        "hours_remaining": time_to_end.total_seconds() / 3600,
                    },
                )
    ...
    async def _get_latest_signal(self, condition_id: str) -> Optional[dict]:
