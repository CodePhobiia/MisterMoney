...
from .entities import EvidenceItem, SourceDocument
...
class EvidenceNormalizer:
    ...
    def extract_claims_deterministic(
        ...
        number_pattern = (
            r'(\w+(?:\s+\w+){0,3})\s+'
            r'(increased|decreased|grew|fell|rose|'
            r'dropped|reached|hit|topped)\s+'
            r'(?:by\s+)?([0-9.]+)'
            r'(%|million|billion|thousand|dollars|cents)'
        )
        ...
        date_pattern = (
            r'(?:on|by|before|after|during|in)\s+'
        ...
        superlative_pattern = (
            r'(first|largest|biggest|smallest|highest'
            r'|lowest|fastest|slowest|best|worst)'
            r'\s+(\w+(?:\s+\w+){0,3})\s+(?:to|in|of|for)'
        )
