...
class V3Integrator:
    ...
    async def get_blended_fair_value(
        ...
        age_seconds = (
            ...
            - signal.generated_at.replace(tzinfo=timezone.utc)
        ).total_seconds()
        ...
        # V3 signal is valid — apply clamping
        metadata["v3_raw"] = signal.p_calibrated
        
        # Convert max_skew_cents to decimal (e.g., 1.0 cents = 0.01)
        max_skew_decimal = self.max_skew_cents / 100.0
        
        # Clamp V3 signal to [book_midpoint - max_skew, book_midpoint + max_skew]
        lower_bound = book_midpoint - max_skew_decimal
        upper_bound = book_midpoint + max_skew_decimal
        
        v3_clamped = max(lower_bound, min(upper_bound, signal.p_calibrated))
