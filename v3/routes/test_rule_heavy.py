...
async def test_government_shutdown_scenario():
    ...
    market = MarketMeta(
        ...
        description=(
            "This market resolves YES if the US federal"
            " government enters a shutdown state"
            " before April 1, 2026."
        ),
        resolution_source="Official government announcements",
        end_date=datetime(2026, 4, 1),
        rules="""
This market resolves YES if the United States federal government \
enters a shutdown state before April 1, 2026, 12:00 PM ET.

**Shutdown Definition:**
A "shutdown" occurs when federal funding lapses for at least one \
federal agency due to Congress failing to pass appropriations bills \
or continuing resolutions. The shutdown must be officially \
acknowledged by the Office of Management and Budget (OMB) or \
the White House.
...
**Edge Cases:**
- If a shutdown begins at 11:59 PM on March 31, 2026, and \
funding is restored at 12:01 AM on April 1, 2026, does this \
count? (Ambiguous — depends on official OMB announcement)
- If shutdown is announced but funding is retroactively \
restored within 1 hour, does this count? (Ambiguous)
- If only the legislative branch shuts down, does this count? (Explicitly NO)

**Resolution Source:**
Official statements from OMB, the White House, or major news \
outlets (NYT, WSJ, WaPo, AP, Reuters).
        """.strip(),
        clarifications=[
            (
                "Clarification 1: A shutdown lasting"
                " less than 1 hour does NOT qualify"
                " unless officially announced by OMB."
            ),
            (
                "Clarification 2: If a shutdown is"
                " announced but funding is restored"
                " retroactively before any furloughs"
                " occur, this does NOT count."
            ),
        ],
    ...
    evidence = [
        EvidenceItem(
            ...
            polarity="YES",
            claim=(
                "House Speaker announces budget"
                " negotiations have stalled, with no"
                " resolution expected before"
                " March 15 deadline."
            ),
            reliability=0.85,
        ),
        EvidenceItem(
            ...
            polarity="NO",
            claim=(
                "Senate Majority Leader says bipartisan"
                " continuing resolution is 'very likely'"
                " to pass by month-end."
            ),
            reliability=0.75,
        ),
        EvidenceItem(
            ...
            polarity="MIXED",
            claim=(
                "White House budget office warns of"
                " potential 'brief lapse' in funding"
                " if deal is not reached by March 20."
            ),
            reliability=0.80,
        ),
        EvidenceItem(
            ...
            polarity="NEUTRAL",
            claim=(
                "Historical data: US government has"
                " shut down 21 times since 1976, with"
                " 4 shutdowns occurring in the past"
                " 10 years."
            ),
            reliability=0.95,
        ),
    ...
    print("Scenario A (Current Market):")
    dr = extended_data['dispute_risk']
    rc = extended_data['rule_clarity']
    print(f"  Dispute Risk: {dr:.1%}, Rule Clarity: {rc:.1%}")
    ...
    print("Scenario B (High-Value Market - $100k notional):")
    print(f"  Dispute Risk: {dr:.1%}, Rule Clarity: {rc:.1%}")
    ...
    print("Scenario C (Clear Rules, Low Dispute):")
    print("  Dispute Risk: 10.0%, Rule Clarity: 90.0%")
    print("  Notional: $10,000, Uncertainty: 15.0%")
    ...
    print("TEST SUMMARY")
    print("=" * 80)
    print("✓ Opus blind pass completed")
    print("✓ GPT-5.4 judge pass completed")
    print("✓ Escalation logic tested (3 scenarios)")
    print("✓ End-to-end execution completed")
    print()
    print("Sample Opus Analysis:")
