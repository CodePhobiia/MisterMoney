...
- Regulatory approval is denied""",
    ...
    rel_min = min(e.reliability for e in evidence)
    rel_max = max(e.reliability for e in evidence)
    print(f"  - Reliability range: {rel_min:.2f} - {rel_max:.2f}")
    print()
    ...
    try:
        ...
        p = synthesis_estimate.p_hat
        u = synthesis_estimate.uncertainty
        print(f"  Probability: {p:.3f} ± {u:.3f}")
    ...
    try:
        ...
        p = challenge_estimate.p_hat
        u = challenge_estimate.uncertainty
        print(f"  Probability: {p:.3f} ± {u:.3f}")
    ...
    try:
        ...
        be = decision.blind_estimate
        print(
            f"  Final Probability:"
            f" {be.p_hat:.3f} ± {be.uncertainty:.3f}"
        )
    ...
    print("\n  Estimate Comparison:")
    se = synthesis_estimate
    ce = challenge_estimate
    be = decision.blind_estimate
    print(
        f"    Synthesis:  {se.p_hat:.3f}"
        f" ± {se.uncertainty:.3f} ({se.model})"
    )
    print(
        f"    Challenge:  {ce.p_hat:.3f}"
        f" ± {ce.uncertainty:.3f} ({ce.model})"
    )
    print(
        f"    Final:      {be.p_hat:.3f}"
        f" ± {be.uncertainty:.3f} ({be.model})"
    )
