...
class RouteCalibrator:
    ...
    def __init__(self, route: Literal["numeric", "simple", "rule", "dossier"]):
        ...
        self.alpha = 1.0  # Cold start: no extremization (identity)
    ...
    def calibrate(self, raw_p: float, features: dict) -> float:
        ...
        # Extremize to correct RLHF hedging (Paper 2 §3)
        from pmm1.math.extremize import extremize
        p_calibrated = extremize(p_calibrated, self.alpha)
    ...
    def update_weights(self, resolved_markets: list[dict]) -> None:
        ...
        for _ in range(iterations):
            # Compute predictions
            predictions = [_sigmoid(sum(b * xi for b, xi in zip(self.beta, x))) for x in features]
        ...
        # Fit extremization alpha (Paper 2 §3)
        from pmm1.math.extremize import fit_alpha as _fit_alpha
        calibrated_probs = [_sigmoid(sum(b * xi for b, xi in zip(self.beta, x))) for x in features]
        self.alpha = _fit_alpha(calibrated_probs, y, bounds=(0.5, 3.0))
