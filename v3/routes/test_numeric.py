...
from v3.evidence.entities import RuleGraph, EvidenceItem
...
def test_classify_numeric_coingecko():
    ...
    result = registry.classify_market(
        ...
        rules="Resolves YES if BTC price on CoinGecko >= $100,000 at any point before Dec 31, 2024 23:59 UTC",
...
def test_classify_dossier():
    ...
    result = registry.classify_market(
        ...
        rules="Resolves YES if multiple credible sources (NASA, ESA, or peer-reviewed journals) confirm detection of extraterrestrial intelligence. Requires verification from independent panels.",
...
def test_barrier_probability_high_volatility():
    ...
    print(f"✓ Barrier probability volatility effect: low={prob_low_vol:.4f}, high={prob_high_vol:.4f}")
