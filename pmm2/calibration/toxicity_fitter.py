...
class ToxicityFitter:
    ...
    async def fit(self) -> tuple[float, float, float] | None:
        ...
        X = []  # [markout_1s, markout_5s, markout_30s]
        y = []  # adverse_pnl
        ...
        for fill in fills:
            ...
            X.append([markout_1s, markout_5s, markout_30s])
        ...
        try:
            ...
            cov = np.dot(X_np.T, X_np)
            cov_y = np.dot(X_np.T, y_np)
            ...
            try:
                ...
                weights_raw = np.linalg.lstsq(X_np, y_np, rcond=None)[0]
            ...
            corr_1s = self._simple_correlation([x[0] for x in X], y)
            corr_5s = self._simple_correlation([x[1] for x in X], y)
            corr_30s = self._simple_correlation([x[2] for x in X], y)
